...
import { savePatientRecord, saveChatMessage } from '../../services/persistence';
import ReactMarkdown from 'react-markdown';
import remarkGfm from 'remark-gfm';
import rehypeHighlight from 'rehype-highlight';

type Msg = { id: string; role: 'system'|'user'|'assistant'; content: string; created_at: number };
...
export const OneClickDesktopChat: React.FC = () => {
  ...
  const push = (role: Msg['role'], content: string, id?: string) => {
    const item: Msg = { id: id || `m_${Date.now()}_${Math.random().toString(36).slice(2,6)}`, role, content, created_at: Date.now() };
  ...
  const start = async () => {
    ...
    try {
      ...
      if (config.oneClick?.showScreenshot) push('system', '截图完成');
      ...
      const res = await apiClient.generateCombinedRecommendationsStream(
        ...
        (chunk) => updateById(recMsgId, (old) => old + chunk),
        types
      );

      const md = (res?.recommendations as any)?.combined || msgs.find(m => m.id===recMsgId)?.content || '';

      // 保存记录 + 对话
      try {
        savePatientRecord({ id: `rec_${Date.now()}`, patient_id: pi.patient_id, patient_name: pi.name, created_at: Date.now(), screenshot: shot.dataUrl, markdown: md, raw: { patient: pi, res } });
        saveChatMessage({ id: `m_${Date.now()}`, session_id: pi.patient_id, role: 'assistant', content: md, created_at: Date.now() });
      } catch {}

      if (config.oneClick?.allowFollowUp) {
        push('system', '可以继续提问，和同一患者上下文继续对话。');
      } else {
        push('system', '流程已完成。');
      }
    } catch (e) {
      push('system', e instanceof Error ? e.message : '流程失败');
    } finally {
      setRunning(false);
    }
  };
  ...
  const send = async () => {
    ...
    const userId = push('user', input.trim());
    setInput('');

    try {
      saveChatMessage({ id: userId, session_id: sessionId || patient.patient_id, role: 'user', content: input.trim(), created_at: Date.now() });
    } catch {}
    ...
    try {
      ...
      try { const u = new URL(endpoint); if (u.pathname.startsWith('/api/')) endpoint = `${u.origin}/v1/chat/completions`; } catch {}

      const system = { role: 'system', content: '你是资深临床智能助手，请严格围绕患者当前病情提供建议。不要输出开头或结尾说明，仅输出内容。' };
      ...
      const user = { role: 'user', content: `患者信息：\n${patientSummary}\n\n问题：\n${input.trim()}\n\n注意：只输出内容，不要任何额外说明。` };

      const headers: Record<string,string> = { 'Content-Type': 'application/json' };
      if (cfg.apiKey) headers['Authorization'] = `Bearer ${cfg.apiKey}`;
      const payload = { model: cfg.model || cfg.diagnosisModel, messages: [system, user], temperature: cfg.temperature || 0.3, max_tokens: cfg.maxTokens || 1200, stream: true } as any;
        ...
        while (true) {
          ...
          for (const part of parts) {
            ...
            try {
              ...
              if (delta) updateById(aId, (old) => old + delta);
            } catch {}
          }
        }
      }
      // Save assistant message
      try { saveChatMessage({ id: aId, session_id: sessionId || patient.patient_id, role: 'assistant', content: msgs.find(m=>m.id===aId)?.content || '', created_at: Date.now() }); } catch {}
    } catch (e) {
      updateById(aId, () => '对话失败');
    }
  };
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="text-sm text-gray-600">{sessionId ? `会话：${sessionId}` : '未开始会话'}</div>
        <div className="space-x-2">
          <button onClick={start} disabled={running} className="px-3 py-1 bg-primary-600 text-white rounded-md disabled:opacity-50"><Zap className="h-4 w-4 inline mr-1"/>一键开始</button>
          <button onClick={resetSession} className="px-3 py-1 bg-gray-600 text-white rounded-md"><Trash2 className="h-4 w-4 inline mr-1"/>清除</button>
        </div>
      </div>
      ...
      {config.oneClick?.allowFollowUp && (
        <div className="flex items-end space-x-2">
          <textarea
            value={input}
            onChange={(e)=>setInput(e.target.value)}
            className="flex-1 border rounded-md px-3 py-2 border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
            placeholder="继续提问..."
            rows={2}
          />
          <button onClick={send} className="px-3 py-2 bg-primary-600 text-white rounded-md">发送</button>
        </div>
      )}
    </div>
  );
