...
- [ ] 1.2.1 合并CSS变量定义 (4小时)
  ...
  - [ ] 删除App.css中的`:root`定义(第27-124行)
  - [ ] 删除App.css中的媒体查询
  - [ ] 验证所有页面样式正常
  - [ ] 运行 `npm run dev` 全面测试

- [ ] 1.2.2 移除废弃样式类 (4小时)
  - [ ] 全局搜索 `dark:glass-dark` 并替换为 `.glass`
  - [ ] 全局搜索 `glass-header` 并替换为语义化类名
  - [ ] 全局搜索 `glass-card` 并替换为语义化类名
  - [ ] 全局搜索 `glass-effect` 并替换为 `.glass`
  - [ ] 验证所有组件样式正常

- [ ] 1.2.3 审计和替换硬编码颜色 (8小时)
  - [ ] 运行 `rg "#[0-9a-fA-F]{6}" --glob "*.tsx"` 找出所有硬编码颜色
  - [ ] 创建颜色映射表(硬编码 -> CSS变量/Tailwind类)
  - [ ] 逐个替换硬编码颜色
  - [ ] 验证主题切换功能正常
  - [ ] 测试深色模式和玻璃模式
